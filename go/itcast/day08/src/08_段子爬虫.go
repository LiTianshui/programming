...
func SpiderOneJoy(url string) (title string, content string, err error) {
	...
	// 取内容
	tmpTitle := re1.FindAllStringSubmatch(result, 1) // 最后一个参数为1，之过滤第一个
	...
	// 取内容
	tmpContent := re2.FindAllStringSubmatch(result, -1) // 最后一个参数为1，之过滤第一个
...
func SpiderPage(i int, page chan int) {
	...
	for _, data := range joyUrls {
		...
		if err != nil {
			fmt.Println("SpiderOneJoye err = ", err)
			continue
		}
